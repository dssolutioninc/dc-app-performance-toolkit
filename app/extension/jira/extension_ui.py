...
def test_1_view_project_page_rr(webdriver, datasets):
    page = BasePage(webdriver)
    project_page_bpn = Project(webdriver, project_key=datasets['current_session']['project_key'])

    @print_timing("selenium_view_project_page_rr")
    def measure_project_page():
        # Project page
        project_page_bpn.go_to_project_page_bpn()
        project_page_bpn.wait_for_project_page_bpn()

    @print_timing("selenium_view_global_page_rr")
    def measure_global_page():
        # Global page
        page.go_to_url(f"{JIRA_SETTINGS.server_url}/secure/admin/RiskRegisterAdminConfigAction.jspa")
